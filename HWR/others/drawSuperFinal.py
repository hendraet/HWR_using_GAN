...
parser = argparse.ArgumentParser('Create final graph')
parser.add_argument('epoch', type=int, help='which epoch')
...
gts = []
for i in img_names:
    with open('RWTH_partition/RWTH.iam_word_gt_final.test.thresh', 'r') as f:
        gt = f.readlines()
        for j in gt:
            ss = j.strip().split(' ')
            if i == ss[0]:
...
def drawSuper(epoch):
    ...
    preds = []
    for i in img_names:
        #print('imgs/test_'+i+'_'+str(epoch)+'.jpg')
        img = cv2.imread('imgs/test_'+i.split(',')[0]+'_'+str(epoch)+'.jpg', 0)
        print('imgs/test'+i.split(',')[0]+'_'+str(epoch)+'.jpg', 0)
        print(img)
