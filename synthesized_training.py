...
parser.add_argument('--test_folder', default='test_images/',
                    help='Folder that contains the images to test the trained model on.')
parser.add_argument('-t', '--test', action='store_true',
                    help='If specified, the model will be tested on images in the test_folder and compared to the '
                         'default model.')
...
if args.iam:
    ...
    # create index file with words of this writer in train_images_names for creating images with GAN
...
if args.test:
    if args.iam:
        test_images(groundtruth_labels, Path(config['iam_words']), trained_model_path, f'writer_{run_id}_')
        ...
        test_images(labels_file, test_folder, trained_model_path, f'run_{run_id}_')
        test_images(labels_file, test_folder, config['hwr_default_model'], f'run_{run_id}_original_')
